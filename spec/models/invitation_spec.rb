...
describe Invitation do
  it "at creation is not accepted" do
    invitation = Invitation.create!(model: create(:model), editor: create(:editor))
  ...
  describe ".resolve_pending" do
    it "should accept the pending invitation of the assigned editor" do
      ...
      Invitation.resolve_pending(pending_invitation.model, pending_invitation.editor)
    ...
    it "should do nothing if invitation already accepted" do
      ...
      expect { Invitation.resolve_pending(invitation.model, invitation.editor) }.to_not change { Invitation.pending.count }
      expect { Invitation.resolve_pending(invitation.model, invitation.editor) }.to_not change { Invitation.accepted.count }
    end

    it "should expire invitations to not assigned editors" do
      invitation_1 = create(:invitation, :pending)
      invitation_2 = create(:invitation, :pending, model: invitation_1.model)
      assigned_editor = create(:editor)

      Invitation.resolve_pending(invitation_1.model, assigned_editor)
    ...
    it "should do nothing if no invitation exists" do
      ...
      expect { Invitation.resolve_pending(create(:model), create(:editor)) }.to_not change { Invitation.pending.count }
      expect { Invitation.resolve_pending(create(:model), create(:editor)) }.to_not change { Invitation.accepted.count }
    end
  end
end
