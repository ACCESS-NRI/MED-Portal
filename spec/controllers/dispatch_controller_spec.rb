...
describe DispatchController, type: :controller do
  ...
  describe "POST #github_receiver for REVIEW with invalid HTTP_X_HUB_SIGNATURE", type: :request do
    before do
      ...
      @model = create(:model, meta_review_issue_id: 78, review_issue_id: 79)
    ...
    it "should detect invalid signature" do
      ...
      expect(@model.activities).to eq({})
    end
  end

  describe "POST #github_receiver for PRE-REVIEW", type: :request do
    before do
      @model = create(:model, meta_review_issue_id: 78, review_issue_id: nil)
    ...
    it "should initialize the activities when an issue is opened" do
      ...
      expect(@model.activities).to eq({"issues"=>{"commenters"=>{"pre-review"=>{}, "review"=>{}}, "comments"=>[], "last_edits"=>{}, "last_comments" => {}}})
    end
    ...
    it "should UPDATE the activities when an issue is then commented on" do
      ...
      expect(response).to be_ok
      expect(@model.activities['issues']['commenters']).to eq({"pre-review"=>{"whedon"=>1, "editor"=>1}, "review"=>{}})
      expect(@model.activities['issues']['comments'].length).to eq(2)
      expect(@model.activities['issues']['last_comments']['editor']).to eq("2018-09-30T11:48:30Z")
      expect(@model.activities['issues']['last_comments']['whedon']).to eq("2018-09-30T11:48:40Z")
    end
  end

  describe "POST #github_receiver for REVIEW with labeling event", type: :request do
    before do
      @model = create(:model, meta_review_issue_id: 78, review_issue_id: 79, labels: [{ "foo" => "efefef" }])
      post '/dispatch', params: whedon_review_labeled, headers: headers(:issues, whedon_review_labeled)
      @model.reload
    end

    it "should update the labels on the model" do
      expect(response).to be_ok
      expect(@model.labels).to eq({ "accepted" => "0052cc" })
    end
  end
  ...
  describe "POST #github_receiver for REVIEW", type: :request, vcr: true do
    ...
    it "should update the last_edits key" do
      expect(response).to be_ok
      expect(@model.activities).to eq({"issues"=>{"commenters"=>{"pre-review"=>{}, "review"=>{}}, "comments"=>[], "last_comments" => {}, "last_edits"=>{"comment-editor"=>"2018-10-06T16:18:56Z"}}})
    end

    it "should update the percent_complete value" do
      expect(@model.percent_complete).to eq(0.9375)
    end
    ...
    it "should update the last_activity field" do
      ...
      expect(@model.last_activity.strftime('%Y-%m-%dT%l:%M:%S%z')).to eql(github_updated_at)
    end
  end

  describe "POST #github_receiver", type: :request do
    it "shouldn't do anything if the payload is not for one of the models" do
      random_model = create(:model, meta_review_issue_id: 1234)
      post '/dispatch', params: whedon_pre_review_comment, headers: headers(:issue_comment, whedon_pre_review_comment)
      random_model.reload

      expect(response).to be_ok
      expect(random_model.activities).to eq({})
    end

    it "shouldn't do anything if a payload is received for the wrong repository" do
      model = create(:model, meta_review_issue_id: 78)
    ...
    it "shouldn't care if an issue comment payload is received before the 'opened' payload" do
      ...
      expect(response).to be_ok
      expect(model.activities['issues']['commenters']).to eq({"pre-review"=>{}, "review"=>{"whedon"=>1}})
      expect(model.activities['issues']['comments'].length).to eq(1)
      expect(model.activities['issues']['commenters']['reviews']).to be_nil
    end
  end
  ...
  describe "POST #api_start_review" do
    ...
    it "with the correct API key, a single reviewer, but an invalid editor" do
      ...
      model = create(:review_pending_model, state: "review_pending", meta_review_issue_id: 1234, user_id: user.id)
      ...
      expect(editor.models.count).to eq(0)
    end
    ...
    it "with the correct API key and a single reviewer" do
      ...
      expect(editor.models.count).to eq(1)
      expect(model.reload.reviewers).to eq(['@mickey'])
    end
    ...
    it "with the correct API key and multiple reviewers" do
      ...
      expect(response).to be_created
      expect(editor.models.count).to eq(1)
      expect(model.reload.reviewers).to eq(['@mickey', '@minnie'])
    end
    ...
    it "with the correct API key and multiple reviewers should strip whitespace" do
      ...
      expect(response).to be_created
      expect(editor.models.count).to eq(1)
      expect(model.reload.reviewers).to eq(['@white', '@space'])
    end
  end
  ...
  describe "POST #api_assign_editor" do
    ...
    it "with the correct API key and valid editor" do
      ...
      model = create(:review_pending_model, state: "review_pending", meta_review_issue_id: 1234)
      ...
      expect(model.reload.editor).to eql(editor)
    end
    ...
    it "with the correct API key and invalid editor" do
      ...
      expect(model.reload.editor).to be_nil
    end

    it "with the correct API key and invalid model" do
      editor = create(:editor, login: "jimmy")
      model = create(:review_pending_model, state: "review_pending", meta_review_issue_id: 1234)
  ...
  describe "POST #api_editor_invite" do
    ...
    it "with the correct API key and valid editor" do
      ...
      expect { post :api_editor_invite, params: post_params }.to change { model.invitations.count }.by(1)
    end
  ...
  describe "POST #api_assign_reviewers" do
    ...
    it "with the correct API key" do
      ...
      expect(response).to be_successful
      expect(model.reload.reviewers).to eql(["@joey", "@dave"])
    end
  ...
  describe "PUT #api_reject" do
    ...
    it "with the correct API key for a review_pending model" do
      model = create(:review_pending_model, state: "review_pending", meta_review_issue_id: 1234)
      ...
      expect(response).to be_successful
      expect(model.reload.state).to eql("rejected")
    end

    it "with the correct API key for rejected model" do
      model = create(:rejected_model, meta_review_issue_id: 1234)
  ...
  describe "PUT #api_withdraw" do
    ...
    it "with the correct API key for a review_pending model" do
      ...
      expect(response).to be_successful
      expect(model.reload.state).to eql("withdrawn")
    end
  ...
  describe "POST #api_deposit" do
    ...
    it "with the correct API key" do
      user = create(:user)
      model = create(:under_review_model, review_issue_id: 1234, user_id: user.id)
      ...
      expect(response).to be_successful
      expect(model.reload.state).to eql('accepted')
      expect(model.metadata['model']['reviewers']).to eql(["@jim", "@bob"])
    end
  end
end
