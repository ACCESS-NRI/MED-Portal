...
RSpec.describe AeicDashboardController, type: :controller do
  ...
  describe "#index" do
    ...
    let(:accepted_issue_1) { OpenStruct.new(number: 1, html_url: "/test1", title: "Accepted model 1") }
    let(:accepted_issue_2) { OpenStruct.new(number: 2, html_url: "/test2", title: "Accepted model 2") }
    let(:flagged_issue_1) { OpenStruct.new(number: 3, html_url: "/test3", title: "Flagged model 1") }
    let(:flagged_issue_2) { OpenStruct.new(number: 4, html_url: "/test4", title: "Flagged model 2") }
    ...
    it "lists all open accepted issues" do
      ...
      expect(response.body).to have_content "Accepted model 1"
      expect(response.body).to have_content "Accepted model 2"
    ...
    it "lists all issues flagged with query-scope" do
      ...
      expect(response.body).to have_content "Flagged model 1"
      expect(response.body).to have_content "Flagged model 2"
